...
const TABS = ['Proyectos', 'Usuarios', 'Asignaturas', 'Solicitudes']
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    // ✅ CORREGIDO: ruta correcta según el backend es /admin/requests/pending
    api.get('/admin/requests/pending')
      .then(res => setRequests(res.data?.data || res.data || []))
      .catch(() => setRequests([]))
      .finally(() => setLoadingRequests(false))

    // ✅ NOTA: /users y /subjects admin no están en las rutas del backend que compartiste.
    // Si las tienes en el backend, funcionarán. Si no, estas llamadas fallarán con 404.
    api.get('/users')
  ...
  const handleApprove = async id => {
    try {
      // ✅ CORREGIDO: ruta correcta /admin/requests/{id}/approve
      await api.post(`/admin/requests/${id}/approve`)
      setRequests(prev => prev.filter(r => r.id !== id))
    } catch { alert('Error al aprobar la solicitud.') }
  }

  const handleReject = async id => {
    try {
      // ✅ CORREGIDO: ruta correcta /admin/requests/{id}/reject
      await api.post(`/admin/requests/${id}/reject`)
      setRequests(prev => prev.filter(r => r.id !== id))
    } catch { alert('Error al rechazar la solicitud.') }
  }
  ...
  const stats = [
    ...
    { label: 'Solicitudes', value: requests.length, icon: '⏳', color: 'bg-yellow-50 text-yellow-700' },
  ]
  ...
  const typeLabel = type => ({
    ...
    'update_project': 'Editar proyecto',
        ...
        {activeTab === 'Solicitudes' && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <h2 className="font-semibold text-gray-900">Solicitudes pendientes ({requests.length})</h2>
            </div>

            {loadingRequests ? (
              <div className="text-center py-16 text-gray-400 text-sm">Cargando solicitudes...</div>
            ) : requests.length === 0 ? (
              <div className="text-center py-16 text-gray-400 text-sm">No hay solicitudes pendientes. ✅</div>
            ) : (
              <div className="divide-y divide-gray-100">
                {requests.map(req => (
                  <div key={req.id} className="px-5 py-4 flex items-start gap-4 hover:bg-gray-50 transition">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-medium text-gray-900">
                        {req.data?.title || req.project?.title || `Solicitud #${req.id}`}
                      </p>
                      {req.data?.description && (
                        <p className="text-xs text-gray-500 mt-0.5 line-clamp-2">{req.data.description}</p>
                      )}
                    </div>
