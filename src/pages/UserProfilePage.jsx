...
export default function UserProfilePage() {
  ...
  useEffect(() => {
    api.get(`/users/${id}/projects`)
      .then(r => { setUserData(r.data.user); setProjects(r.data.projects) })
    ...
    <div className="min-h-screen bg-gray-50"><Navbar />
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-8 flex items-center gap-5">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">{userData.name || userData.email}</h1>
            {userData.name && <p className="text-sm text-gray-400">{userData.email}</p>}
